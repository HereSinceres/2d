...
export default class LineGeometry extends BufferGeometry {
	...
	update(path: Array<number[]>, closed = false) {
		...
		let attrPosition = this.getAttribute('position') as any;
		let attrNormal = this.getAttribute('lineNormal') as any;
		let attrMiter = this.getAttribute('lineMiter') as any;
		let attrDistance = this.getAttribute('lineDistance') as any;
		let attrIndex = this.getIndex() as any;
		...
		attrPosition = attrPosition as any;
